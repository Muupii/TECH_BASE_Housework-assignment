...
if ($houseworksDoneAction->count($_SESSION['user'], $year, $month, $day) != null) {
    $count = $houseworksDoneAction->count($_SESSION['user'], $year, $month, $day);
} else {
    $count = 0;
}
...
<body>
        ...
        <h2><?= $loginUser['name'] ?>さんの<?= $month ?>月<?= $day ?>日に行った家事についてのページです</h2>
        <div class="wrapper">
            <h2>家事登録フォーム</h2>
            <h3><?= $loginUser['name'] ?>さんの<?= $month ?>月<?= $day ?>日に行った家事を覚えている限り一つずつ入力してください。</h3>

            
            
            <form action="housework_done.php" id="hausework_done" method="post">
            ...
            if ($count > 0): 
                ...
                <div class="wrapper">
                    ...
                    <?php foreach ($houseworks as $housework):
                            ...
                            if ($_SESSION['user'] == $housework['user_id'] && mb_substr($housework['start_date'], 0, 10) == $year . "-" . $month . "-" . $day): 
                                if ($housework['is_noname'] == 0): 
                                    $sum_point++; ?>
                        <p><?php echo "名もなき家事：" . $housework['housework'] . "　取得ポイント： 1pt　"; ?><a href="delete_housework_done.php?id=<?=$housework['id']?>&month=<?= $month ?>&day=<?=$day?>&year=<?=$year?>" class="btn">削除</a></p>
                    ...
                    <?php else: ?>
                        ...
                        <p><?= "家事：" . $housework['housework'] . "　開始時間：" . mb_substr($housework['start_date'], 11, 5) . "　終了時間：" . mb_substr($housework['finish_date'], 11, 5) . "　取得ポイント：" . round($point, 2) . "pt　" ?> <a href="delete_housework_done.php?id=<?=$housework['id']?>&month=<?= $month ?>&day=<?=$day?>&year=<?=$year?>" class="btn">削除</a></p>    
        ...
        <?php           
                ...
                endforeach; ?>
                <br>
                <a href="all_family_houseworks_done.php?month=<?= $month ?>&day=<?=$day?>&year=<?=$year?>" class="btn"">他の家族も含めた<?= $month ?>月<?= $day ?>日に行った家事一覧</a>
                </div>
        <?php endif;    ?>

        <div class="wrapper">
            <h2>取得ポイント合計</h2>
            <h3><?= round($sum_point, 2) ?>ポイント</h3>
            <p>※30分＝1ポイント、ただし、名もなき家事は労働時間にかかわらず1ポイント</p>
        </div>
