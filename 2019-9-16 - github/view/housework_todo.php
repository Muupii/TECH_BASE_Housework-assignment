...
if(count($errors) === 0){
    ...
    //ここで家事登録する
    if ($flag == "yes") {
        $houseworksToDoAction->register_noname($_SESSION['user'], $housework, $year, $month, $day);
        header("Location: future_day_show.php?year={$year}&month={$month}&day={$day}");
    } else {
        $houseworksToDoAction->register($_SESSION['user'], $housework, $year, $month, $day, $start_time, $finish_time);
        header("Location: future_day_show.php?year={$year}&month={$month}&day={$day}");
    }
}
